...
# pull a list of rookie RBs with at least 20 carries per game, get their opportunities, 
rbList <- c()
...
rbDF.fixedcols$opportunities <- rbDF.fixedcols$Att + rbDF.fixedcols$Tgt

# find RBs with max opportunities for their respective ages
maxAtts <- c()
for (i in unique(rbDF.fixedcols$Age)) {
  ageDF <- subset(rbDF.fixedcols, Age == i)
  pn <- ageDF$Player[ageDF$opportunities == max(ageDF$opportunities)]
  maxAtts[[length(maxAtts) + 1]] <- pn
  rm(ageDF, pn)
}

# clean out anyone who wouldn't make the final figure interesting/useful
rbDF.fixedcols <- subset(rbDF.fixedcols, opportunities >= 50)

# make the plot
g <- ggplot(data = rbDF.fixedcols, 
             mapping = aes(x = Age, y = opportunities, label = paste0(Player, 
                                                                ' (', opportunities, 
                                                                ')'))) +
  ...
  ylab('Opportunities (Rushing Attempts + Passing Targets)') +
  ggtitle(label = 'Total Opportunities, Rookie Season, NFL Running Backs, By Rookie Season Age', subtitle = '(Minimum 50 Opportunities; Ages In Years; Labeled Points Are Players With Most Rookie opportunities For That Age)') +
  theme_minimal() +
  theme(panel.grid.minor = element_blank())
# see it
g
# save it
ggsave('NFL_RB_RookieOpportunitiesAge.jpg', plot = g, units = 'in',
       width = 10, height = 5)  
